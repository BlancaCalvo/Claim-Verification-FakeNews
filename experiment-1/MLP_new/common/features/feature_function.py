...
class Features():
    ...
    def load(self,train,dev=None,test=None):
        ...
        if self.check_needs_generate(train,dev,test): 
            self.inform(train,dev,test) #problema aquí
        else:
            try:
        ...
        # STEP 1
        for ff in self.feature_functions: #ff is the sentencelevelterm frequency function
            # so basically here it would take as many feature functions as you give it and apply them to the corpuses
    ...
    def generate_or_load(self,feature,dataset,name):
        ...
        if dataset is not None:
            # if the features already exist load them
            if os.path.exists(os.path.join(ffpath,name)) and os.getenv("GENERATE","").lower() not in ["y","1","t","yes"]:
                ...
                #print(feature)
                features = feature.lookup(dataset.data) #it refers to the lookup function of gever_features
...
class FeatureFunction(): #this does nothing that I can see
