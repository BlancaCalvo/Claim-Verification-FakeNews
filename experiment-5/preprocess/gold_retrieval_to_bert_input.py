        ...
        for evidence in evidences:
            ...
            for row in cursor:
                ...
                for sentence in sentences:
                    ...
                    #sentence = ' '.join(arr[1:]) # DELETE NAMED ENTITIES
                    sentence = arr[1]
                    if sentence == '':
                        continue
            ...
            if evidence_str:
                fout.write(
                    ('%s\t%s\t%s\t%s\t%s\t%d\t%s\r\n'
                     % (label, evidence_str, claim, index, article, location,threshold)
                     ).encode(ENCODING)
                )
            else:
        ...
        for evidence_set in evidences:
            ...
            for evidence in evidence_set:
                ...
                for row in cursor:
                    ...
                    #sentence = ' '.join(items[1]) # DELETE NAMED ENTITIES
                    sentence = items[1]
...
if __name__ == '__main__':
    #build_bert_train_sr_set('data/fever/train.jsonl', 'data/gear/gold_bert-nli-train-sr-set.tsv')
    process('data/retrieved/train.ensembles.s10.jsonl', 'data/gear/gold_bert-nli-train-retrieve-set.tsv')
    process('data/retrieved/dev.ensembles.s10.jsonl', 'data/gear/gold_bert-nli-dev-retrieve-set.tsv')
