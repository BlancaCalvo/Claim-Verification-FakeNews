...
## Results from sembert with Attention layer between propositions (sembert_att)
...
heatmap_errors(aggregation_results)

```

### Comparison base_bert and sembert_att

```{r ce9}

d<-table_results(bert_results, aggregation_results)
...
```{r ce15, include=FALSE, eval=FALSE}
f_bert_results<- read.csv("C:/Users/bcalvo/Box/bcalvo/Claim-Verification-FakeNews/experiment-5/outputs/bert_base/F-dev-results.tsv", header = FALSE, sep='\t')

f_bert_results <- f_bert_results %>%
  #filter(V4!=2)%>%
  rowwise() %>%
  mutate(output= which.max(list(V1,V2,V3))-1)%>%
  mutate(right = sum(V4==output, na.rm=T))

print(sum(f_bert_results$right)/nrow(f_bert_results))

f_bert_results <- cbind(f_bert_results, V5 = dev$V1) 
...
```{r ce16}
att_tag<- read.csv("C:/Users/bcalvo/Box/bcalvo/Claim-Verification-FakeNews/experiment-5/outputs/agg_tags1/dev-results.tsv", header = FALSE, sep='\t')
