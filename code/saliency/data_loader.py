...
_FEVER_DIC_LABELS = {'NOTENOUGHINFO': 2, 'REFUTES': 1, 'SUPPORTS': 0}
...
def get_dataset(path, dataset='snli', mode='test'):
    if dataset == 'snli':
        ...
        ds = FEVERDataset(path, type=mode, salient_features=True)
    elif dataset == 'tweet':
...
class FEVERDataset(Dataset):
    def __init__(self, dir, type='train', sample_dev=False,
        ...
        self.salient_features = salient_features
        #for _path in [os.path.join(dir, _p) for _p in self._PATHS[type]]:
        with open(dir) as out:
            self._dataset.extend([line for line in
                                  csv.reader(out, delimiter='\t')])
    ...
    def __getitem__(self, item):
        result = [self._dataset[item][2],
                  #self._dataset[item][3],
                  ' '.join(self._dataset[item][3:]),
                  _FEVER_DIC_LABELS[self._dataset[item][1]]]
        #if self.salient_features: # WHAT ARE THESE SALIENT FEATURES????
        #    result += [self._dataset[item][5], self._dataset[item][6],
        #               self._dataset[item][7], self._dataset[item][8]]
        #print(result)
        return tuple(result)
