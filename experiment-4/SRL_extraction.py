...
if __name__ == "__main__":
    ...
    with open(args.input_file, "r", encoding='utf-8') as reader:
        while True:
            ...
            claim = line[2]
            evidences = line[3:]
            claim_prediction = predictor.predict_json({'sentence': claim})['verbs']

            for evidence in evidences:
                evidence = re.sub(r'\.[a-zA-Z \-é0-9\(\)]*$', '',
                                  evidence)  # instead of this line I should change the build_gear_input_set.py script
                try:
                    prediction = predictor.predict_json({'sentence': evidence})
                except RuntimeError:
                    print('Length issue with this evidence: ', evidence)
                ...
                json_list.append(
                    {'unique_id': unique_id, 'claim': claim, 'claim_srl': claim_prediction, 'evidence': evidence, 'evidence_srl': prediction['verbs'],
